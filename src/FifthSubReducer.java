...
public class FifthSubReducer extends
	...
	protected void reduce(NullWritable key, Iterable<Text> values,
		...
		Map<String, Integer> carrierToDelay = new HashMap<String, Integer>();
		Map<String, Integer> carrierToMinutesWasted = new HashMap<String, Integer>();
		Map<String, Float> carrierToAverageDelayMinutes = new HashMap<String, Float>();
		...
		for (Text allValues : values) {
			...
			carrierToDelay.put(airportToCityName[0] + "-"
					+ airportToCityName[1], Integer.parseInt(parsed[1]));
			carrierToMinutesWasted.put(airportToCityName[0] + "-"
					+ airportToCityName[1], Integer.parseInt(parsed[2]));

			if (Integer.parseInt(parsed[1]) != 0)
				carrierToAverageDelayMinutes.put(airportToCityName[0] + "-"
						+ airportToCityName[1], (float) (Integer
						.parseInt(parsed[2]) * 1.0 / Integer
						.parseInt(parsed[1])));

		}

		Map<String, Integer> sortedCarrierToDelay = sortByValues(carrierToDelay);
		Map<String, Integer> sortedCarrierToMinutesWasted = sortByValues(carrierToMinutesWasted);
		Map<String, Float> sortedCarrierToAverageDelayMinutes = sortByFloatValues(carrierToAverageDelayMinutes);

		int count = 10;
		String answerStringFor4 = "Question 5\nCarriers With Higher Delays\n\n**According To Total Number Of Delays**\n";
