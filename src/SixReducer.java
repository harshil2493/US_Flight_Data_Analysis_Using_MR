...
public class SixReducer extends
		Reducer<Text, SixCustomizedDataType, NullWritable, Text> {
	...
	protected void reduce(Text key, Iterable<SixCustomizedDataType> values,
		...
		for (SixCustomizedDataType allValues : values) {
			if (allValues.getFlag().get()) {
				// context.write(key, allValues);
				if (manYear == 0)
					manYear = allValues.getDataLine().get();
			} else {
		...
		if (!(manYear == 0)) {
			for (Integer currentYear : yearlyDelayedCount.keySet()) {
				// SixCustomizedDataType sixCustomizedDataType = new
				...
				// sixCustomizedDataType.setDataLine(new
				// IntWritable(currentYear));

				context.write(nullWrite,
